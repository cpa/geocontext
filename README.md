...
| `GPF_ITINERAIRE_RATE_LIMIT`  | Nombre maximum de requêtes par seconde sur le service de calcul d'itinéraire de la Géoplateforme                                                                                                                                                                                                  | 5                                                |
...
* [itineraire(start_lon,start_lat,end_lon,end_lat,profile)](src/tools/ItineraireTool.ts) s'appuie sur le [service de calcul d'itinéraire de la Géoplateforme](https://cartes.gouv.fr/aide/fr/guides-utilisateur/utiliser-les-services-de-la-geoplateforme/calcul-itineraire/) pour **calculer un trajet voiture ou piéton** avec la ressource `bdtopo-osrm`. Le `profile` est explicite : si le mode n'est pas donné, l'assistant doit choisir `pedestrian` sous 2 km, `car` au-dessus de 5 km, et demander à l'utilisateur entre les deux avant d'appeler le tool. Le résultat contient la géométrie GeoJSON de l'itinéraire et, quand le service les renvoie, les étapes de guidage dans `properties.turnByTurn`.

> Ex : Donne-moi l'itinéraire à pied entre le Louvre et la Bastille.
